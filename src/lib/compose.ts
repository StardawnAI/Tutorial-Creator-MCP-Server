...
function joinSegments(segments: RawSegment[], fps: number, transitionSec: number): string {
  ...
  const parts = kept.map((segment, k) => {
    const holdForNext = dissolves[k + 1] ? `,tpad=stop_mode=clone:stop=${fadeFrames}` : ''
    return (
      ...
      `setsar=1,format=yuv420p,${COLOUR},fps=${fps}[s${segment.index}]`
    )
  })
  ...
  kept.slice(1).forEach((segment, j) => {
    ...
    // xfade also drops the colour description, so it is declared again here.
    const restamp = `setpts=N/(${fps}*TB),${COLOUR},fps=${fps}`
    parts.push(
      dissolves[k]
 ...
 *
 * Stated outright because otherwise the route through the graph decides it. A
 * render with hard cuts came out tagged tv/bt470bg, one with dissolves untagged -
 * xfade drops the description - and a player handed an untagged HD file usually
 * assumes BT.709, which shifts the colours a little. Declared here, every render is
 * tagged the same way.
 */
const COLOUR = 'setparams=range=tv:colorspace=bt470bg'
